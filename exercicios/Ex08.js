...
//             maior = resultado.indexOf[i]
//         } else if (resultado[i] < resultado[i - 1]){
//             menor = resultado.indexOf[i]
//         }
//     }
//     return maior
// }
...
function record(stringPontos){
    let resultado = stringPontos.split(", ")
    let maior = 0
    let menor = 1
    let maiorPontuacao = resultado[0]
    let menorPontuacao = resultado[0]
    for(let i = 1; i < resultado.length; i++){
        if(resultado[i] > maiorPontuacao){
...
console.log(record(stringPontos))